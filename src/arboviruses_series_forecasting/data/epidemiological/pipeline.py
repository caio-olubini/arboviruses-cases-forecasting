...
PANEL_COLUMNS = ("uf", "week_start", "cases", "ew_year", "ew", "log10_cases")
...
def build_dengue_uf_ew_series(
    ...
    *,
    ...
    Returns the model-ready panel schema written to ``dengue_uf_ew.parquet``.
    """
    ...
    panel = complete_uf_week_panel(observed)
    panel = pd.concat([panel, epiweek_of(panel["week_start"])], axis=1)
    panel["log10_cases"] = np.log10(panel["cases"] + 1)
